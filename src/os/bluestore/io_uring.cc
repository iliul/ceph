...
int ioring_queue_t::init(std::vector<int> &fds)
{
  ...
  ret = io_uring_queue_init(iodepth, &d->io_uring, flags);
  if (ret < 0)
    return ret;
                                 ...
                                 int *retries)
{
  ...
  rc = ioring_queue(d, priv, beg, end);
...
get_cqe:
  ...
  events = ioring_get_cqe(d, max, paio);
  pthread_mutex_unlock(&d->cq_mutex);

  if (events == 0) {
    ret = epoll_wait(d->epoll_fd, &ev, 1, timeout_ms);
    if (ret < 0)
      events = -errno;
    else if (ret > 0)
      /* Time to reap */
